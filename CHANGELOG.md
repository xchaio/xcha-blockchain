...
- Mainnet uses port 5120 and other constants and service names were changed for mainnet.
...
- Testnet is now running at the primary port of 55120. Update your routers appropriately. This opens 5120 for mainnet.
...
Graphical installer. Try `./xcha -h` from
...
"xcha plots create" and "xcha plots check".
- Add plot directories to config.yaml from the cli with "xcha plots add".
...
- New AJAX based full node UI. To access go to [http://127.0.0.1:5155/index.html](http://127.0.0.1:5155/index.html) with any modern web browser on the same machine as the full node.
...
- Implemented an RPC interface with JSON serialization for streamables - currently on port 5155.
...
- Introducer now makes sure it only sends peer addresses to peers of peers that it can reach on port 5120 or their UPnP port.
