...
if [ ! "$1" ]; then
  echo "This script requires either amd64 of arm64 as an argument"
	...
	DIR_NAME="xcha-blockchain-linux-x64"
else
	#PLATFORM="$1"
	DIR_NAME="xcha-blockchain-linux-arm64"
fi
...
electron-packager . xcha-blockchain --asar.unpack="**/daemon/**" --platform=linux \
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create xcha-blockchain-$XCHA_INSTALLER_VERSION.rpm"
  ...
  --arch "$REDHAT_PLATFORM" --options.version $XCHA_INSTALLER_VERSION \
  --license ../LICENSE
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
