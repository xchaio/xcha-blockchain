...
export const mojo_to_chia = (mojo) => {
  return chia_formatter(Number.parseInt(mojo), 'mojo').to('xcha').value();
};

export const chia_to_mojo = (chia) => {
  return chia_formatter(Number.parseFloat(Number(chia)), 'xcha')
    .to('mojo')
    .value();
};

export const mojo_to_chia_string = (mojo) => {
  return chia_formatter(Number(mojo), 'mojo').to('xcha').toString();
};
