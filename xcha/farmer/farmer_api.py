...
from xcha.consensus.network_type import NetworkType
from xcha.consensus.pot_iterations import calculate_iterations_quality, calculate_sp_interval_iters
from xcha.farmer.farmer import Farmer
from xcha.protocols import farmer_protocol, harvester_protocol
from xcha.protocols.harvester_protocol import PoolDifficulty
from xcha.protocols.pool_protocol import (
...
from xcha.util.api_decorators import api_request, peer_required
...
class FarmerAPI:
    ...
    async def respond_signatures(self, response: harvester_protocol.RespondSignatures):
        ...
        xcha_pool_sp = None
        xcha_pool_sp_sig = None
        agg_sig_xcha_pool_sig = None
            ...
            for sk in self.farmer.get_private_keys():
                ...
                if pk == response.farmer_pk:
                    ...
                    if include_taproot:
                        ...
                        if xcha_pool_sp is not None:
                            taproot_share_xcha_sp: G2Element = AugSchemeMPL.sign(
                                taproot_sk, xcha_pool_sp, agg_pk)

                    else:
                        taproot_share_cc_sp = G2Element()
                        taproot_share_rc_sp = G2Element()
                        taproot_share_xcha_sp = G2Element()
                    ...
                    if pospace.pool_public_key is not None:
                        ...
                        if xcha_pool_sp is not None and xcha_pool_sp_sig is not None and self.farmer.pool_target is not None:
                            xcha_share_sp = AugSchemeMPL.sign(
                                sk, xcha_pool_sp, agg_pk)
                            agg_sig_xcha_pool_sig = AugSchemeMPL.aggregate(
                                [xcha_pool_sp_sig,
                                 xcha_share_sp, taproot_share_xcha_sp]
                            )
                            assert AugSchemeMPL.verify(
                                agg_pk, xcha_pool_sp, agg_sig_xcha_pool_sig)
                            pool_target: Optional[PoolTarget] = PoolTarget(
                                self.farmer.pool_target, uint32(0))
                            assert pool_target is not None
                            pool_target_signature = agg_sig_xcha_pool_sig
                        else:
                            pool_target = None
