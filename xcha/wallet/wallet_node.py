...
from xcha.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from xcha.protocols import wallet_protocol
from xcha.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from xcha.protocols.protocol_message_types import ProtocolMessageTypes
from xcha.protocols.wallet_protocol import (
...
from xcha.server.node_discovery import WalletPeers
from xcha.server.outbound_message import Message, NodeType, make_msg
from xcha.server.server import ChiaServer
from xcha.server.ws_connection import WSChiaConnection
from xcha.types.blockchain_format.coin import Coin, hash_coin_list
...
from xcha.types.peer_info import PeerInfo
from xcha.util.byte_types import hexstr_to_bytes
from xcha.util.errors import Err, ValidationError
from xcha.util.ints import uint32, uint128
from xcha.util.keychain import Keychain
from xcha.util.lru_cache import LRUCache
from xcha.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from xcha.wallet.settings.settings_objects import BackupInitialized
...
from xcha.wallet.wallet_blockchain import ReceiveBlockResult
from xcha.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'xcha keys' program.")
            return None
