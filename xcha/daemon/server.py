...
from xcha.cmds.init_funcs import chia_init
from xcha.daemon.windows_signal import kill
from xcha.server.server import ssl_context_for_root, ssl_context_for_server
from xcha.ssl.create_ssl import get_mozilla_ca_crt
from xcha.util.chia_logging import initialize_logging
from xcha.util.config import load_config
from xcha.util.json_util import dict_to_json_str
from xcha.util.path import mkdir
from xcha.util.service_groups import validate_service
from xcha.util.setproctitle import setproctitle
from xcha.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Xcha.")
    quit()
...
service_plotter = "xcha plots create"
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("xcha daemon exiting in 5 seconds")
