...
ROOT = pathlib.Path(importlib.import_module("xcha").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `xcha` file name (which is the executable)
    and the `xcha` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `xcha` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("xcha-blockchain"))[0]
...
entry_points = ["xcha.cmds.xcha"] + [f"xcha.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "xcha.util.bip39"])

if THIS_IS_WINDOWS:
    chia_mod = importlib.import_module("xcha")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/xcha/util/english.txt", "xcha/util"))
datas.append((f"{ROOT}/xcha/util/initial-config.yaml", "xcha/util"))
datas.append((f"{ROOT}/xcha/wallet/puzzles/*.hex", "xcha/wallet/puzzles"))
datas.append((f"{ROOT}/xcha/ssl/*", "xcha/ssl"))
...
add_binary("xcha", f"{ROOT}/xcha/cmds/xcha.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/xcha/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/xcha/server/start_{server}.py", COLLECT_ARGS)
