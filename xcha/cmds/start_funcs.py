...
from xcha.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from xcha.util.service_groups import services_for_groups
...
def launch_start_daemon(root_path: Path) -> subprocess.Popen:
    ...
    xcha = sys.argv[0]
    process = subprocess.Popen(f"{xcha} run_daemon".split(), stdout=subprocess.PIPE)
    return process
...
async def async_start(root_path: Path, group: str, restart: bool) -> None:
    ...
    if daemon is None:
        print("Failed to create the xcha daemon")
        return None
