...
from xcha.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'xcha keys generate'")
        return None
...
def create_all_ssl(root: Path):
    ...
    chia_ca_crt_path = ca_dir / "xcha_ca.crt"
    chia_ca_key_path = ca_dir / "xcha_ca.key"
...
def chia_init(root_path: Path):
    if os.environ.get("CHIA_ROOT", None) is not None:
        print(
            ...
            f"Please unset the environment variable and run xcha init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Xcha directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if CHIA_ROOT is set, or if user has run xcha init twice
        # before a new update.
    ...
    check_keys(root_path)
    print("")
    print("To see your keys, run 'xcha keys show --show-mnemonic-seed'")
