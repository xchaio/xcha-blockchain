...
from xcha.util.ints import uint16
from xcha.util.misc import format_bytes
from xcha.util.misc import format_minutes
from xcha.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total xcha farmed: {amounts['farmed_amount'] / units['xcha']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['xcha']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['xcha']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'xcha start wallet' and 'xcha wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'xcha wallet show'")
    else:
        print("Note: log into your key using 'xcha wallet show' to see rewards for each key")
