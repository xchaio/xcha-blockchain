...
def show_plots(root_path: Path):
    from xcha.plotting.plot_tools import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'xcha plots add -d [dir]' and remove with"
        + " 'xcha plots remove -d [dir]'"
        + " Scan and check plots with 'xcha plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'xcha init'")
...
def create_cmd(
    ...
    from xcha.plotting.create_plots import create_plots
...
def check_cmd(
    ...
    from xcha.plotting.check_plots import check_plots
...
def add_cmd(ctx: click.Context, final_dir: str):
    from xcha.plotting.plot_tools import add_plot_directory
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from xcha.plotting.plot_tools import remove_plot_directory
